...
#average weight gain
STg<- signif(mean(STtagRT$weight_end-STtagRT$Weight), 3)
#g<- "x"
#average length growth
STl<- signif(mean(STtagRT$length_end-STtagRT$Length), 3)
#l<- "y"

#Count shed and mort by study
STshedcount<- count(sheds[which(sheds$StudyID==study),])
SSmortcount<- count(morts[which(morts$StudyID==study),])
```
...
```{r ttabold, eval=FALSE,  include=FALSE, warning=FALSE}
