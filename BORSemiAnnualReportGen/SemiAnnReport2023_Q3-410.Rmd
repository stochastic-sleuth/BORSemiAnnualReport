...
knitr::opts_knit$set(root.dir = '~/Desktop/BORSemiAnn2024Q1-2')

# ! need to troubleshoot line 1054
...
This report summarizes the fieldwork, data collection, and analysis performed by UC Santa Cruz (UCSC) between April 1 and September 30, 2023, as part of the Cooperative Agreement R21AC10455 between the US Bureau of Reclamation (USBR) and UCSC. This is the third semi-annual report for the Cooperative Agreement R21AC10455, extending from April 1, 2021 to September 30, 2026. This semi-annual report outlines deliverables for the six tasks described by the agreement.
...
tag <- TaggedFish %>%
              ...
              filter(DateTagged >= as.POSIXct(begindate)) #
...
BStag_table<- plyr::ddply(tagbs, .(DateTagged), summarise, count=sum(fish_number), meanW=mean(Weight), meanL=mean(Length), 
...
```{r, sutterfish, echo=FALSE}
#Select studies to look at
tagsut<-tag[which(tag$StudyID=="SB_Spring_2023"),]

sutter<-unique(tagsut[,c("StudyID","Fish_Type", "Rel_loc","DateTagged","Rel_datetime")])

tagsut$fish_number<-1

#Create summary table
SBtag_table<- plyr::ddply(tagsut, .(DateTagged), summarise, count=sum(fish_number), meanW=mean(Weight), meanL=mean(Length), 
                 SD_W=sd(Weight), SD_L=sd(Length), min_W=min(Weight), min_L=min(Length),
                 max_W=max(Weight), max_L=max(Length))

#Set decimal place to 2
SBtag4<-as.data.frame(lapply(SBtag_table[sapply(SBtag_table, is.numeric)], round, 2))
#Add back in rel datetime row
SBtag4$DateTagged<-SBtag_table$DateTagged

SBtag5<-merge(SBtag4,sutter)

SBtag5<-SBtag5[order(SBtag5$Rel_datetime),]

#Reduced table for report
SBtag6<-SBtag5[,c("StudyID", "Fish_Type", "Rel_loc", "count", "Rel_datetime", "meanW", "meanL")]
names(SBtag6)[names(SBtag6)=="Rel_loc"]<-"Release Loc"
names(SBtag6)[names(SBtag6)=="count"]<-"Count"
names(SBtag6)[names(SBtag6)=="Rel_datetime"]<-"Release Date"
names(SBtag6)[names(SBtag6)=="meanW"]<-"Avg Weight"
names(SBtag6)[names(SBtag6)=="meanL"]<-"Avg Length"
names(SBtag6)[names(SBtag6)=="Fish_Type"]<-"Fish Type"

suttertotal<- sum(SBtag6$Count)
```

UCSC staff implanted acoustic tags into `r buttetotal` wild-caught fish in Butte Creek, and `r suttertotal` in the Sutter Bypass during this period.

```{r table5, echo=FALSE }

wildtag6<- rbind(BStag6, SBtag6)
...
## Tag Life Test
```{r wildTLstart, echo = FALSE}
# ###Read in startup files
# # List Startup Spec Sheet Filenames
# knitr::opts_knit$set(root.dir = '/Users/jessefrey/Desktop/BORSemiAnnualReportQ3Q4DESK/Tag_Life/StartUpSpecSheets/')
setwd<- "/Users/jessefrey/Desktop/BORSemiAnnualReportQ3Q4DESK/Tag_Life/StartUpSpecSheets/"
files<- list.files("/Users/jessefrey/Desktop/BORSemiAnnualReportQ3Q4DESK/Tag_Life/StartUpSpecSheets/")

ButteTLstart<- read_csv("~/Desktop/BORSemiAnnualReportQ3Q4DESK/Tag_Life/StartUpSpecSheets/Butte_2023_Tag_Life.csv")
ButteTLstart$Notes<- as.character(ButteTLstart$Notes)

#
ButteTLstart$Date<- as.Date(ButteTLstart$DateTime,format = "%m/%d/%Y")
#remove deactivated tag
ButteTLstart<- ButteTLstart[!grepl("DEACTIVATED", ButteTLstart$Notes),]
#round PRI
ButteTLstart$PRI<- round(ButteTLstart$PRI, digits = 0)

###Create Tag Model and warranty life dataframe
Tag_Model<-c("SS300","SS400","SS300 bat 392","SS400 2 bat 379","SS300","SS400","SS300 bat 392","SS400 2 bat 379","SS300","SS400","SS300 bat 392","SS400 2 bat 379","SS300","SS400","SS300 bat 392","SS400 2 bat 379")
PRI<-c(3,3,3,3,5,5,5,5,8,8,8,8,10,10,10,10)
warranty_life<-c(23, 48, 79, 999, 37, 71, 128, 999, 999, 90, 999, 999, 68, 111, 238, 999)
warranty<-data.frame(Tag_Model, PRI, warranty_life)

#Add warranty life and warranty date to ButteTLstart table
ButteTLstart3<-left_join(ButteTLstart, warranty) %>%
  mutate(warranty_date = Date + warranty_life)

colnames(ButteTLstart3)[colnames(ButteTLstart3) == "Date"] ="start_date"

#tags started
ButteTLtagcount<- nrow(ButteTLstart)
#average tag life in days
ButteTLavgrun<- signif(mean(ButteTLstart$days),3)
#min tag life in days
ButteTLmi<- signif(min(ButteTLstart$days),3)
#max tag life in days
ButteTLma<- signif(max(ButteTLstart$days),3)
```
To monitor the battery life of the tags used tagging wild Chinook in the Sutter Bypass, a 5% random sample was taken from the total proportion of tags used for each release group. In total, X SS400 tags were started over a period of 4 weeks (on 12/13/2022, 2/01/2023,3/01/2023, and 03/15/2023) and placed in the tag life tank located at the NMFS-SWFSC lab for monitoring. Data collected in this study examined the range of battery life for these particular tags, in order to correct any discrepancies in survival estimates as a result of tags shutting off prematurely. X tags in the X wild Chinook tag life study made it to the warranty life of 71 days. The average run time was X days with a range of X to X days.

```{r ButteTLresults, echo = FALSE}
#
...
```{r SSfish, eval=FALSE, include=FALSE}
...
#Select studies to look at
tagsp<-tag[which(tag$StudyID=="Spring_Pulse_2023"),]

sp2<-unique(tagsp[,c("StudyID","Fish_Type","Raceway", "Rel_loc","DateTagged","Rel_datetime")])

tagsp$fish_number<-1
tagsp<- tagsp %>% drop_na(Weight)

#Create summary table
SPtag_table<- plyr::ddply(tagsp, .(Raceway, DateTagged), summarise, 
                 count=sum(fish_number+1), meanW=mean(Weight), meanL=mean(Length), 
                 SD_W=sd(Weight), SD_L=sd(Length), min_W=min(Weight), min_L=min(Length),
                 max_W=max(Weight), max_L=max(Length))

#Set decimal place to 2
SPtag4<-as.data.frame(lapply(SPtag_table[sapply(SPtag_table, is.numeric)], round, 2))
#Add back in rel datetime row
SPtag4$DateTagged<-SPtag_table$DateTagged

SPtag5<-merge(SPtag4,sp2)
...
During this period UCSC staff implanted acoustic tags into hatchery produced late-fall run Chinook smolts as part of the 2023 Spring Pulse Flow study between dates..

```{r table6, echo=FALSE }
#pipe data to table and give caption
SPtag6<- SPtag6%>%
  flextable() %>%
    set_caption(caption = "Table 6. Hatchery fish acoustic tagged for the Spring Pulse Flow Study.") %>%
    font(fontname = "Times New Roman", part = "all") %>%
    fontsize(size = 11, part = "body") %>% 
    width(width = .75)
...
start<-read.csv("~/Desktop/BORSemiAnnualReportQ3Q4DESK/Tag_Effects/SS_2023_Tag_Effects_Start.csv", header=TRUE)
#read in data from date dissection
end<-read.csv("~/Desktop/BORSemiAnnualReportQ3Q4DESK/Tag_Effects/SS_2023_Tag_Effects_End.csv", header=TRUE)
...
end2$Tag.ID<- str_replace(end2$Tag.ID, "84C1", "B4C1")
end2$Tag.ID<- str_replace(end2$Tag.ID, "71F4", "71E4")
shed2<- shed %>% 
...
As part of the Seasonal Survival Study, a total of `r sstotal` juvenile winter run and fall-run Chinook salmon were tagged at CNFH and LSNFH across three weeks (12/12/2022, 1/31/2023, and 3/14/2023) and transported to the NMFS-SWFSC lab where they were held on-site. The fish were fed and the tanks checked daily for expelled tags. Fish were measured and weighed at the start and end of the trial. At the end of the six week tag retention study, all fish were processed and inspected for shed tags. During this trial, `r SSshedcount` tags were found shed. Four tags were recovered from fish mortalities, one seven days after tagging, and three between 15 and 58 days from tagging due to the development of tail rot during holding.

The average weight gained was `r SSg` and the average length gained was `r SSl`.

```{r sptr, echo = FALSE}
#summarise spring pulse
SPtagRT<- tagRTfish2[which(tagRTfish2$StudyID=="Spring_Pulse_2023"),]

sptotal<- count(SPtagRT)
#what happened to fish 59AB?
SPtagRT<- SPtagRT[which(SPtagRT$Tag.ID!="59AB"),]

#average weight gain
SPg<- signif(mean(SPtagRT$weight_end-SPtagRT$Weight), 3)
#g<- "x"
#average length growth
SPl<- signif(mean(SPtagRT$length_end-SPtagRT$Length), 3)
#l<- "y"


#Count shed and mort by study
SPshedcount<- count(sheds[which(sheds$StudyID=="Spring_Pulse_2023"),])
SPmortcount<- count(morts[which(morts$StudyID=="Spring_Pulse_2023"),])
```

### Spring Pulse Tag Retention Study

As part of the Spring Pulse Study, a total of `r sptotal` juvenile fall-run Chinook salmon were tagged at CNFH across three weeks (12/12/2022, 1/31/2023, and 3/14/2023) and transported to the NMFS-SWFSC lab where they were held on-site. The fish were fed and the tanks checked daily for expelled tags. Fish were measured and weighed at the start and end of the trial. At the end of the six week tag retention study, all fish were processed and inspected for shed tags. During this trial, `r SPshedcount` tags were found shed. One tag was recovered from a fish mortality the morning immediately following surgery.

The average weight gained was `r SPg` and the average length gained was `r SPl`.

### San Joaquin Steelhead Tag Retention Study

Hatchery produced Steelhead were unable to be sourced and tagged at the Mokelumne River Hatchery during this report period. No wild-caught Steelhead were retained for tag retention studies.
...
setwd("~/Desktop/BORSemiAnnualReportQ3Q4DESK/Tag_Life/StartUpSpecSheets")
...
```{r, readfiltered_detects, echo = FALSE, warning=FALSE}
...
SPTLplot
```
To monitor the battery life of the tags used for the Spring Pulse Study, a 5% random sample was taken from the total proportion of tags used for each release group. In total, `r tagcount` SS400  and `r` SS300 tags were started over a period of 4 weeks (on 12/13/2022, 2/01/2023,3/01/2023, and 03/15/2023) and placed in the tag life tank located at the NMFS-SWFSC lab for monitoring. Data collected in this study examined the range of battery life for these particular tags, in order to correct any discrepancies in survival estimates as a result of tags shutting off prematurely. `r n` tags in the 2023 Seasonal Survival tag life study made it to the warranty life of 71 days. The average run time was `r avgrun` days with a range of `r mi` to `r ma` days.


### San Joaquin Steelhead Tag Life Study

To monitor the battery life of the tags used for the San Joaquin Steelhead, a 5% random sample was taken from the total proportion of tags used for each release group. In total, `r tagcount` SS400 tags were started over a period of 4 weeks (on 12/13/2022, 2/01/2023,3/01/2023, and 03/15/2023) and placed in the tag life tank located at the NMFS-SWFSC lab for monitoring. Data collected in this study examined the range of battery life for these particular tags, in order to correct any discrepancies in survival estimates as a result of tags shutting off prematurely. `r n` tags in the 2023 Seasonal Survival tag life study made it to the warranty life of 71 days. The average run time was `r avgrun` days with a range of `r mi` to `r ma` days.

The average weight gained was `r g` and the average length gained was `r l`.
