...
params:
  printcode: no
  begindate: "April 1, 2024"
  enddate: "September 30, 2024"
subtitle: "Semi-annual report `r params$begindate` to `r params$enddate`"
output:
  word_document:
...
This report summarizes the fieldwork, data collection, and analysis performed by UC Santa Cruz (UCSC) between `r begindatechar` and `r  enddatechar`, as part of the Cooperative Agreement R21AC10455 between the US Bureau of Reclamation (USBR) and UCSC. This is the fourth semi-annual report for the Cooperative Agreement R21AC10455, extending from April 1, 2021 to September 30, 2026. This semi-annual report outlines deliverables for the six tasks described by the agreement.
...
Rec_Dep2<- Rec_Dep2[Rec_Dep2$Location != "Anywhere", ]
...
Rec_Loc2<-subset(Rec_Loc, select = c("GeneralLocation","rkm", "GPSname", "Region", "GenLat", "GenLon"))
#join receiver deployments and receiver locations 
Rec_Dep4<-unique(merge(Rec_Dep3,Rec_Loc2, all.x=TRUE))
Rec_Dep4$GenLat<- with(Rec_Dep4, ifelse(is.na(GenLat), LatWater, GenLat))
Rec_Dep4$GenLat<- with(Rec_Dep4, ifelse(is.na(GenLat), LatShore, GenLat))
Rec_Dep4$GenLon<- with(Rec_Dep4, ifelse(is.na(GenLon), LonWater, GenLon))
Rec_Dep4$GenLon<- with(Rec_Dep4, ifelse(is.na(GenLon), LonShore, GenLon))
#reorder columns
Rec_Dep5<-Rec_Dep4[,c("Region","GPSname","GenLat","GenLon","rkm","RecMake","SN","StartTime","EndTime")]
...
There were `r nrow(Rec_Dep6)` real-time receivers deployed and/or retrieved during this report period (Table 1). All acoustic receivers stationed in the Lower Sacramento River (`r sac`), Stanislaus River (`r st`), and Carquinez Straight (`r cs`) were retrieved and deployed by UCSC.
...
##Table 2. Autonomous JSATS receivers active, deployed, or retrieved between report dates
...
# note these are stored without a region variable, unlike Autonomous
Vemco_Dep2<-subset(Vemco_Dep, select = c("GPSname", "VemcoSN", "StartTime", "EndTime", "LatShore", "LonShore", "LatWater", "LonWater"))
...
Vemco_Dep3<- subset(Vemco_Dep3, !is.na(GPSname))

test2<- left_join(Vemco_Dep3, Rec_Loc, by="GPSname")
vemco_missing_GENLOC<- test2[which(is.na(test2$GeneralLocation), arr.ind=TRUE),]
...
Vemco_Dep4$GenLat<- with(Vemco_Dep4, ifelse(is.na(GenLat), LatWater, GenLat))
Vemco_Dep4$GenLat<- with(Vemco_Dep4, ifelse(is.na(GenLat), LatShore, GenLat))
Vemco_Dep4$GenLon<- with(Vemco_Dep4, ifelse(is.na(GenLon), LonWater, GenLon))
Vemco_Dep4$GenLon<- with(Vemco_Dep4, ifelse(is.na(GenLon), LonShore, GenLon))

#Order table by start time
Vemco_Dep4<-Vemco_Dep4[order(as.Date(Vemco_Dep4$StartTime, format="%d-%m-%Y")),]

Vemco_Dep5<-subset(Vemco_Dep4, select = c("GPSname","GenLat", "GenLon", "Genrkm","VemcoSN",
                          "StartTime","EndTime")) %>%
  dplyr::rename("Lat" = "GenLat") %>%
  dplyr::rename("Lon" = "GenLon") %>%
...
#### Butte Sink Study
```{r buttefish, echo=FALSE }

#Select studies to look at
tagbs<-tag[which(tag$StudyID=="ButteSink_2024"),]

butte<-unique(tagbs[,c("StudyID","Fish_Type","Rel_loc","DateTagged","Rel_datetime")])

tagbs$fish_number<-1

#Create summary table
BStag_table<- plyr::ddply(tagbs, .(DateTagged), summarise, count=sum(fish_number), 
                          meanW=mean(Weight), meanL=mean(Length), SD_W=sd(Weight), 
                          SD_L=sd(Length), min_W=min(Weight), min_L=min(Length),
                          max_W=max(Weight), max_L=max(Length))

#Set decimal place to 2
BStag4<-as.data.frame(lapply(BStag_table[sapply(BStag_table, is.numeric)], round, 2))
#Add back in rel datetime row
BStag4$DateTagged<-BStag_table$DateTagged

BStag5<-merge(BStag4,butte)

BStag5<-BStag5[order(BStag5$Rel_datetime),]

#Reduced table for report
BStag6<-BStag5[,c("StudyID", "Fish_Type", "Rel_loc", "count", "Rel_datetime", "meanW", "meanL")]
names(BStag6)[names(BStag6)=="Rel_loc"]<-"Release Loc"
names(BStag6)[names(BStag6)=="count"]<-"Count"
names(BStag6)[names(BStag6)=="Rel_datetime"]<-"Release Date"
names(BStag6)[names(BStag6)=="meanW"]<-"Avg Weight"
names(BStag6)[names(BStag6)=="meanL"]<-"Avg Length"
names(BStag6)[names(BStag6)=="Fish_Type"]<-"Fish Type"

buttetotal<- sum(BStag6$Count)
...
wildtag6<- wildtag6%>%
  flextable() %>%
    set_caption(caption = "Table 4. Wild Chinook acoustic tagged during this report period.") %>%
    font(fontname = "Times New Roman", part = "all") %>%
    fontsize(size = 11, part = "body") %>% 
    width(width = .75)

#set table style
wildtag6<- theme_zebra(wildtag6, odd_header="#4472c4", odd_body ="#d9e2f3", even_header="transparent", even_body="transparent")
wildtag6<- border_outer(wildtag6, part="all", border= bord)
wildtag6<- border_inner(wildtag6, part="body", border=bord)
wildtag6<- align(wildtag6, align = "center", part = "all")
...
s1$DateTagged<- gsub("17:00:00", "",s1$DateTagged)
...
#collect day vars
s2<-unique(tagss[,c("StudyID","Fish_Type","Raceway", "Rel_loc","DateTagged","Rel_datetime", "Rel_group")])

#add fish number to count fish
tagss$fish_number<-1

#Create summary table
SStag_table<- plyr::ddply(tagss, .(DateTagged, Rel_group), summarise, count=sum(fish_number), 
...
SStag5<-left_join(SStag4, s2, by="Rel_group")
...
UCSC staff implanted `r sstotal` acoustic tags into hatchery produced jumpstart winter-run and late-fall run Chinook smolts as part of the Seasonal Survival Study during this report period. Releases of these fish were spaced out across five weeks, with one week of tagging occurring during the third quarter on 4/01/24 and 4/02/24 with the intention of gathering movement and survival data across a range of environmental conditions during winter. These fish were tagged at Coleman Fish Hatchery and transported to the Red Bluff Diversion Dam (RBDD_Rel) to increase the sample size of fish in downstream reaches. Fish were available for tagging with the help and assistance of the U.S. Fish and Wildlife Service. Tagged fish are shown in Table 4.

Preliminary movement and survival data can be found here: <https://oceanview.pfeg.noaa.gov/CalFishTrack/pageSeasSurv_2024.html>
...
```
```{r pulsefish, echo=FALSE }
study<- paste("Spring_Pulse_", WY, sep = "")

#Select studies to look at
tagsp<-tag[which(tag$StudyID==study),]

tagsp$fish_number<-1
tagsp<- tagsp %>% drop_na(Weight)

# #cleanup date for week vars
sp2<-unique(tagsp[,c("StudyID","Fish_Type","Raceway", "Rel_loc","DateTagged","Rel_datetime", "Rel_group")])
sp2$DateTagged<- gsub("17:00:00", "",sp2$DateTagged)
sp2<-  arrange(sp2, DateTagged)
wk1<- paste(str_trim(sp2[1,5]))
wk2<- paste(str_trim(sp2[3,5]))
wk3<- paste(str_trim(sp2[5,5]))
wk4<- paste(str_trim(sp2[7,5]))

#Create summary table
SPtag_table<- plyr::ddply(tagsp, .(DateTagged, Rel_group), summarise, 
                 count=sum(fish_number+1), meanW=mean(Weight), meanL=mean(Length), 
                 SD_W=sd(Weight), SD_L=sd(Length), min_W=min(Weight), min_L=min(Length),
                 max_W=max(Weight), max_L=max(Length))
...
SPtag5<-left_join(SPtag4,sp2, by="Rel_group")

# SPtag5<-SPtag5[order(SPtag5$Rel_datetime),]

#Reduced table for report
SPtag6<-SPtag5[,c("StudyID", "Fish_Type", "Rel_loc", "Raceway", "count", "Rel_datetime", "meanW", "meanL")]
names(SPtag6)[names(SPtag6)=="Rel_loc"]<-"Release Loc"
names(SPtag6)[names(SPtag6)=="count"]<-"Count"
names(SPtag6)[names(SPtag6)=="Rel_datetime"]<-"Release Date"
names(SPtag6)[names(SPtag6)=="meanW"]<-"Avg Weight"
names(SPtag6)[names(SPtag6)=="meanL"]<-"Avg Length"
names(SPtag6)[names(SPtag6)=="Fish_Type"]<-"Fish Type"

sptotal<- sum(SPtag6$Count)

```

#### Spring Pulse Flow Study

During this period UCSC staff implanted `sptotal` acoustic tags into hatchery produced late-fall run Chinook smolts as part of the 2024 Spring Pulse Flow study between 5/05/2024 and 5/27/2024.

```{r table5, echo=FALSE }
#pipe data to table and give caption
SPtag6<- SPtag6%>%
  flextable() %>%
    set_caption(caption = "Table 5. Hatchery fish acoustic tagged for the Spring Pulse Flow Study.") %>%
    font(fontname = "Times New Roman", part = "all") %>%
    fontsize(size = 11, part = "body") %>% 
    width(width = .75)

#set table style
SPtag6<- theme_zebra(SPtag6, odd_header="#4472c4", odd_body ="#d9e2f3", even_header="transparent", even_body="transparent")
SPtag6<- border_outer(SPtag6, part="all", border= bord)
SPtag6<- border_inner(SPtag6, part="body", border=bord)
SPtag6<- align(SPtag6, align = "center", part = "all")
...
```{r STfish, echo=FALSE}
study<- paste("SJ_Steelhead_", WY, sep = "")

#Select studies to look at
tagst<-tag[which(tag$StudyID==study),]
st1<-s[which(s$StudyID==study),]
st1$DateTagged<- gsub("17:00:00", "",st1$DateTagged)
...
#Create summary table
STtag_table<- plyr::ddply(tagst,.(Rel_group),summarise, count=sum(fish_number), 
                 meanW=mean(Weight), meanL=mean(Length), 
                 SD_W=sd(Weight), SD_L=sd(Length), min_W=min(Weight), min_L=min(Length),
                 max_W=max(Weight), max_L=max(Length))
...
STtag4<- left_join(STtag4, st2, by="Rel_group")

# STtag5<-STtag4[,c("Raceway", "Rel_group","meanL", "count", "DateTagged", "StudyID")]
...
# 
STtag5<-STtag4[order(STtag4$DateTagged),]
...
UCSC staff implanted `r sttotal` acoustic tags into hatchery produced Steelhead at Mokelumne Fish Hatchery and transported to three release sites at Head of Old River, Durham Ferry, and Dos Reis on the San Joaquin River during two weeks staring 4/15/24 and 5/0624. Fish were available for tagging with the help and assistance of the U.S. Fish and Wildlife Service. The first release group of tagged fish is shown in Table 5. 

Preliminary movement and survival data can be found here: <https://oceanview.pfeg.noaa.gov/CalFishTrack/pageSJSTH_2024.html>
...
```{r trtags, eval=FALSE, echo=TRUE}
## download the tag effects fish data set
library(plyr)

TE_start<- tbl(con, "Tag_Effects_Start") %>%
                    collect()
TE_end<- tbl(con, "Tag_Effects_End") %>%
                    collect()

##read in tag shed date, this will need to be a csv
# shed<- read.csv("~/Desktop/BORSemiAnnualReportQ3Q4DESK/Tag_Effects/Tag_Effects_Shed_Mort_List.csv", header = T)


##format colnames
TE_start<- TE_start %>% 
  subset(select=c("TagID_Hex","DateTagged","Weight_Start","Length_Start","StudyID"))

TE_end<- TE_end %>% 
  subset(select=c("TagID_Hex","Date_Euthanized","Weight_End","Length_End","StudyID"))

##format dates
TE_start$DateTagged<-as.Date(TE_start$DateTagged, format = "%m/%d/%Y")
###TE_end$Date_Euthenized<-as.Date(TE_end$Date_Euthenized, format = "%m/%d/%Y")

# shed$Date_Recovered<-as.Date(shed$Date_Recovered, format = "%m/%d/%Y")

##select studies in report WY
TE_start<- subset(TE_start, grepl(".*24", TE_start$StudyID))
#join beginning and end by acoustic tag ID
tagRTfish<- left_join(TE_start, TE_end)

#join shed codes and dates
#tagRTfish2<- full_join(tagRTfish, shed)
...
```{r sstr, eval=FALSE, include=TRUE}
#summarise seasonal survival tag retention
study<- paste("Seasonal_Survival", WY, sep = "")
SStagRT<- tagRTfish2[which(tagRTfish2$StudyID==study),]

sstotalRT<- nrow(SStagRT)
...
SSshedcount<- count(sheds[which(sheds$StudyID==study),])
SSmortcount<- count(morts[which(morts$StudyID==study),])
```

#### Seasonal Survival Tag Retention Study

As part of the Seasonal Survival Study, a total of #r sstotalRT# juvenile winter run and fall-run Chinook salmon were tagged at CNFH and LSNFH across eight weeks (`r wk1`, `r wk2`, `r wk3`, and `r wk4`). Fall-run Chinook were transported to the NMFS-SWFSC lab where they were held on-site. Winter-run fish remained at LSNFH. The fish were fed and the tanks checked daily for expelled tags. Fish were measured and weighed at the start and end of the six week tag retention study. At the end of the study, all fish were processed and inspected for shed tags.

During this report period no tags were found shed. 

For fall-run Chinook tagged during the first week of the Seasonal Survival Study, the average weight gained was #r SSg# and the average length gained was #r SSl# during the tag warranty duration of 71 days.

```{r sttr, eval=FALSE, echo=TRUE}
#summarise steelhead
study<- paste("Steelhead_", WY, sep = "")
STtagRT<- tagRTfish2[which(tagRTfish2$StudyID==study),]

sttotalRT<- nrow(STtagRT)

#no weight or length included since it falls outside of the scope of this report
#how many weeks will this be over?
```
#### San Joaquin Steelhead Tag Retention Study

As part of the Steelhead Survival Study, a total of #r sttotalRT# juvenile fish were tagged at Mokelumne Fish Hatchery and transported to the NMFS-SWFSC lab where they were held on-site. Fish were fed and the tanks checked daily for expelled tags, and were measured and weighed at the trial start. At the end of the >>six week..needs update<< tag retention study, all fish will be measured and weighed, processed and inspected for shed tags. 

## Tag Life Tests

#### Seasonal Surival Tag Life Study

```{r ttabold, eval=FALSE, echo=FALSE, warning=FALSE}
...
```{r ttab, echo=FALSE}
#got lazy and made one sheet with RR varnames, since there was no common format
#this should be replaced by another access table
...
```{r readfiltered_detects, eval=FALSE, warning=FALSE, echo=TRUE}
...
```{r daysdetected, eval=FALSE, echo=TRUE}
...
```{r SSTLdatedead, eval=FALSE, echo=TRUE}
...
```{r SSTLweek, echo=TRUE, eval=FALSE}
...
##### *Tags started week 1:* `r wk1` 
A total of 10 acoustic tags (model SS400) were randomly selected to be used in this tag life study. Tags were started on `r wk1` and placed into the tag life tank for the duration of the study. All tags in the RBDD Week 1 tag life study made it to the warranty life of 71 days and were detected consistently.

##### *Tags started week 2:* `r wk2` 
A total of 10 acoustic tags (model SS400) were randomly selected to be used in this tag life study. Tags were started on `r wk2` and placed into the tag life tank for the duration of the study. 

##### *Tags started week 3:* `r wk3` 
A total of 10 acoustic tags (model SS400) were randomly selected to be used in this tag life study. Tags were started on `r wk3` and placed into the tag life tank for the duration of the study. 

##### *Tags started week 4:* `r wk4` 
A total of 10 acoustic tags (model SS400) were randomly selected to be used in this tag life study. Tags were started on #r wk4# and placed into the tag life tank for the duration of the study. 

```{r, w1, echo=TRUE, eval=FALSE}
