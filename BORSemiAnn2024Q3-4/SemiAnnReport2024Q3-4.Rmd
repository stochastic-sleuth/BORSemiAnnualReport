...
knitr::opts_knit$set(root.dir = '~/Desktop/BORSemiAnnualReport/BORSemiAnn2024Q3-4')
...
##Table 2. Autonomous JSATS receivers active, deployed, or retrieved between begindate and enddate
...
#STtag5$StudyID<- study

STtag5<-STtag5[order(STtag5$DateTagged),]

#Reduced table for report
STtag6<-STtag5[,c("Rel_group", "Raceway", "count...4", "DateTagged", "meanW", "meanL")]
STtag6<- join(STtag6, st2)
STtag6<-STtag6[,c("StudyID", "Fish_Type", "Rel_loc", "Raceway", "count...4", "Rel_datetime", "meanW", "meanL")]
names(STtag6)[names(STtag6)=="Rel_loc"]<-"Release Loc"
names(STtag6)[names(STtag6)=="count...4"]<-"Count"
...
##format colnames
TE_start<- TE_start %>% 
  subset(select=c("Tag.ID","DateTagged","Weight","Length","StudyID"))
# TE_end<- TE_end %>% 
...
# shed$DateTagged<-as.Date(shed$DateTagged, format = "%m/%d/%Y")
...
#tagRTfish<- full_join(TE_start, TE_end)
...
SStagRTwk4<- read.csv("/Volumes/JRFREYKEYS/Week\ 4/wk4_tag_effects.csv")
